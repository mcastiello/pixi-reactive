...
const BlackAndWhiteEffectDoc: React.FC = () => {
  const [enabled, setEnabled] = useState(true);
  const toggle = useCallback(() => setEnabled(!enabled), [enabled]);

  return (
    <>
      <StyledTitle>BlackAndWhiteEffect</StyledTitle>
      <StyledIntroduction>This effect will convert the color matrix to black and white.</StyledIntroduction>
      <Block style={{ textAlign: 'right' }}>
        <span style={{ marginRight: 10 }}>{'Toggle Filter'}</span>
        <Toggle defaultChecked onChange={toggle} />
      </Block>
      <Block style={{ height: 300 }}>
        <BlackAndWhiteEffectExample enabled={enabled} />
      </Block>
      <Block>
        <CodeViewer src={'./static/examples/BlackAndWhiteEffectExample.tsx'} />
      </Block>
...
export default BlackAndWhiteEffectDoc;
