...
const BrowniEffectDoc: React.FC = () => {
  const [enabled, setEnabled] = useState(true);
  const toggle = useCallback(() => setEnabled(!enabled), [enabled]);

  return (
    <>
      <StyledTitle>BrowniEffect</StyledTitle>
      <StyledIntroduction>This effect will make everything a bit more brownish.</StyledIntroduction>
      <Block style={{ textAlign: 'right' }}>
        <span style={{ marginRight: 10 }}>{'Toggle Filter'}</span>
        <Toggle defaultChecked onChange={toggle} />
      </Block>
      <Block style={{ height: 300 }}>
        <BrowniEffectExample enabled={enabled} />
      </Block>
      <Block>
        <CodeViewer src={'./static/examples/BrowniEffectExample.tsx'} />
      </Block>
...
export default BrowniEffectDoc;
