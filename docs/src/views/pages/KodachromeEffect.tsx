...
import { PixiCanvas, PixiSprite, PixiTilingSprite, ColorMatrixFilter, KodachromeEffect } from 'pixi-reactive';
...
const KodachromeEffectExample: React.FC<{ enabled: boolean }> = ({ enabled }) => {
...
const KodachromeEffectDoc: React.FC = () => {
  const [enabled, setEnabled] = useState(true);
  const toggle = useCallback(() => setEnabled(!enabled), [enabled]);

  return (
    <>
      <StyledTitle>KodachromeEffect</StyledTitle>
      <StyledIntroduction>This effect will make the scene look like it was taken with a Kodachrome film.</StyledIntroduction>
      <Block style={{ textAlign: 'right' }}>
        <span style={{ marginRight: 10 }}>{'Toggle Filter'}</span>
        <Toggle defaultChecked onChange={toggle} />
      </Block>
      <Block style={{ height: 300 }}>
        <KodachromeEffectExample enabled={enabled} />
      </Block>
      <Block>
        <CodeViewer src={'./static/examples/KodachromeEffectExample.tsx'} />
      </Block>
...
export default KodachromeEffectDoc;
