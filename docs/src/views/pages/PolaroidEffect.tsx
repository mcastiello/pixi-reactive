...
import { PixiCanvas, PixiSprite, PixiTilingSprite, ColorMatrixFilter, PolaroidEffect } from 'pixi-reactive';
...
const PolaroidEffectExample: React.FC<{ enabled: boolean }> = ({ enabled }) => {
...
const PolaroidEffectDoc: React.FC = () => {
  const [enabled, setEnabled] = useState(true);
  const toggle = useCallback(() => setEnabled(!enabled), [enabled]);

  return (
    <>
      <StyledTitle>PolaroidEffect</StyledTitle>
      <StyledIntroduction>This effect will make the scene look like it was taken with a Polaroid film.</StyledIntroduction>
      <Block style={{ textAlign: 'right' }}>
        <span style={{ marginRight: 10 }}>{'Toggle Filter'}</span>
        <Toggle defaultChecked onChange={toggle} />
      </Block>
      <Block style={{ height: 300 }}>
        <PolaroidEffectExample enabled={enabled} />
      </Block>
      <Block>
        <CodeViewer src={'./static/examples/PolaroidEffectExample.tsx'} />
      </Block>
...
export default PolaroidEffectDoc;
